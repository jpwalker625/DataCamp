...
In this section I'll dive into **regular expressions** which are a language for describing patterns in strings. Rather than using the tradiitional characters and symbols you may have seen before, I'll be introducing the `rebus` package which provides regular expressions in a human readable format.  
...
In this first example we'll use the `words` vector to count how many words there are for a specific repeated letter sequence.
...
str_subset(words, pattern, match = T)

#extract the repeating letters from each word and create table
table(str_extract(string = words, pattern = pattern))
```

Back referencing is especially useful in cases where we want to replace a string using pattern matching.
