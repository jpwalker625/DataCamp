...
Using the `animations` package, motion charts can be made a few ways.

First, by making a series of plots via a for loop and saving them as a GIF.
...
p <- ggplot(sunspots, aes(x = year, y = sunspots, group = month, frame = month))+
  geom_line()

gganimate(p, interval = 1.2)

#create GIF object
saveGIF({
 
  for(i in unique(sunspots$month)){
    
    #filter sunspots for each month
    data <- filter(sunspots, month == i)
    
    #creat plot object
    p <- ggplot(data = data, aes(x = year, y = sunspots))+
      geom_line(aes(group = month))+
      ggtitle(i)
    
    print(p)
  }
}, movie.name = "sunspots.GIF", img.name = "sunspots", interval = .1)
