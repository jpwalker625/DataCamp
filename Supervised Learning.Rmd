...
#create new column
biopsy <- biopsy %>% mutate(malignant = 0)

#assign True or False to whether the class is malignant
biopsy$malignant <- case_when(biopsy$class == "malignant" ~ TRUE,
            biopsy$class != "malignant" ~ FALSE)

biopsy.fmla <- as.formula("malignant ~ .")

logit_mod <- glm(formula = biopsy.fmla, family = binomial, data = biopsy[ , -10])
...
#plot the gain curve
GainCurvePlot(frame = biopsy, xvar = "pred", truthVar = "malignant", title = "Biopsy class model")
```
